...
def evaluate_nesp(save_subfolder):
    ...
    for prefix in ['reliable_', 'unreliable_']:
        ...
        columns = ['Peak demand [kW]', 'Peak demand NESP [kW]', 'Peak demand diff [%]',
                   ...
                   'RES [%]', 'Supply reliability extension [%]', 'Customers [-]', 'Population [-]',
                   'Distance to national grid [km]', 'Additional branch distance [km]',
                   'LCOE [USD/kWh]', 'NPV [USD]', 'NPV/HH [USD/HH]', 'NPV/kW [USD/kW]',
                   'LCOE extension [USD/kWh]', 'NPV extension [USD]', 'NPV/HH extension [USD/HH]',
                   'NPV/kW extension [USD/kW]'
                   ]
        ...
        for file in folder_list:
            ...
            for item in data.index:
                ...
                if case == 'offgrid_mg':
                    ...
                    capacities_stage['NPV/kW [USD/kW]'][nesp_id] = capacities_stage['NPV [USD]'][nesp_id] / \
                                                                   data['demand_peak_kW'][item] * 1000
            ...
            capacities_stage['Peak demand diff [%]'] = (capacities_stage['Peak demand [kW]'] - capacities_stage[
                'Peak demand NESP [kW]']) / \
            ...
            capacities_stage['Diff NPV/kW [USD/kW]'] = capacities_stage['NPV/kW [USD/kW]'].values - capacities_stage[
                'NPV/kW extension [USD/kW]'].values
            capacities_stage['Diff NPV/HH [USD/HH]'] = capacities_stage['NPV/HH [USD/HH]'].values - capacities_stage[
                'NPV/HH extension [USD/HH]'].values
            capacities_stage['Diff NPV [USD]'] = capacities_stage['NPV [USD]'].values - capacities_stage[
                'NPV extension [USD]'].values
            capacities_stage['Diff LCOE [USD/kWh]'] = capacities_stage['LCOE [USD/kWh]'].values - capacities_stage[
                'LCOE extension [USD/kWh]']
        ...
        number = plot_matrix(folder, prefix, capacities, number, 'LCOE [USD/kWh]')
        ...
        plots = pd.DataFrame([capacities[name].values for name in
                             ...
                             index=['Additional branch distance [km]', 'Distance to national grid [km]',
                                    'Peak demand [kW]']).transpose()
        for column in plots.columns:
            q = plots[column].quantile(0.90)
            plots = plots[plots[column] < q]

        number = number + 1
        fig = plots.plot.hexbin(x='Additional branch distance [km]', y='Peak demand [kW]', colormap='viridis',
                                gridsize=15)
        ...
        number = number + 1
        fig = plots.plot.hexbin(x='Distance to national grid [km]', y='Peak demand [kW]', colormap='viridis',
                                gridsize=15)
        fig.set(title="Distribution of project sites")
        plt.savefig(folder + save_subfolder + 'graph_' + prefix + str(number) + '.png', bbox_inches="tight")
        ...
        number = plot_x_y_stages(folder, prefix, comparison_list_x, comparison_list_y, stage1, stage2, stage3, no_mgs, number)

def plot_comparison_intercon(folder, prefix, comparison_list_x, comparison_list_y, costs, number, perspective):
...
def add_sum_save(folder, prefix, number_of_favoured, name, perspective):
...
def plot_x_y_stages(folder, prefix, comparison_list_x, comparison_list_y, stage1, stage2, stage3, no_mgs, number):
...
def plot_matrix(folder, prefix, matrix, number, c):
...
def evaluate_grid_arrival_time(grid_arrival, save_subfolder):
    ...
    for prefix in ['reliable_', 'unreliable_']:
        ...
        case_list_aspect2_on_off_perspectives = [name + ' global perspective' for name in list_cases] + [
            name + ' mg perspective' for name in case_list_aspect2_on_off]
        case_list_aspect2_on_perspectives = [name + ' global perspective' for name in list_cases] + [
        ...
        number_of_favoured = pd.DataFrame(np.zeros((len(folder_list), len(list_cases) * 2)), index=folder_list,
                                          columns=[name + ' global perspective' for name in list_cases] + [
        ...
        for file in folder_list:
            ...
            for item in ['offgrid_mg', 'sole_maingrid', 'offgrid_mg_cons', 'offgrid_mg_cons_prod', 'ongrid_mg_cons',
                ...
                if item == 'sole_maingrid':
                    ...
                    costs_stage['NPV [USD] ' + item + ' mg perspective'] = \
                        costs_stage['NPV [USD] offgrid_mg'] * crf_t + costs_stage['NPV [USD] ' + item] / crf_20_t / d_t \
            ...
            for perspective in ['mg perspective', 'global perspective']:
                for item in list_cases:
                    costs_stage['NPV/kW [USD/kW] ' + item + ' '+perspective] = \
                        costs_stage['NPV [USD] ' + item + ' '+perspective] / \
                        costs_stage['Peak demand [kW]']

                    costs_stage['NPV/HH [USD/HH] ' + item + ' '+perspective] = \
                        costs_stage['NPV [USD] ' + item + ' '+perspective] / \
                ...
                favoured, number_of_favoured_aspect3_on_off \
                    = compare_solutions(costs_stage, file, favoured, number_of_favoured_aspect3_on_off, 'favoured_aspect2_on',
                                        case_list_aspect3_on_off, 'LCOE [USD/kWh] ', perspective)
                ...
                favoured, number_of_favoured_aspect3_adapt_lcoe \
                    = compare_solutions(costs_stage, file, favoured, number_of_favoured_aspect3_adapt_lcoe,
                                        'favoured_aspect3_adapt_lcoe', case_list_aspect3_adapt, 'LCOE [USD/kWh] ',
                                        perspective)
            # print(favoured)
        ...
        add_sum_save(folder, prefix, number_of_favoured_aspect1[[name + ' global perspective' for name in case_list_aspect1]],
                     name_list[0], 'global perspective')
        add_sum_save(folder, prefix,
        ...
        add_sum_save(folder, prefix, number_of_favoured_aspect2_on[[name + ' global perspective' for name in case_list_aspect2_on]],
        ...
        add_sum_save(folder, prefix, number_of_favoured_aspect3_on[[name + ' global perspective' for name in case_list_aspect3_on]],
                     name_list[4], 'global perspective')
        add_sum_save(folder, prefix, number_of_favoured_aspect3_npv[[name + ' global perspective' for name in case_list_aspect3_all]],
                     name_list[5], 'global perspective')
        add_sum_save(folder, prefix, number_of_favoured_aspect3_lcoe[[name + ' global perspective' for name in case_list_aspect3_all]],
        ...
        add_sum_save(folder, prefix, number_of_favoured_aspect1[[name + ' mg perspective' for name in case_list_aspect1]], name_list[0],
                     'mg perspective')
        add_sum_save(folder, prefix, number_of_favoured_aspect2_on_off[[name + ' mg perspective' for name in case_list_aspect2_on_off]],
                     name_list[1], 'mg perspective')
        add_sum_save(folder, prefix, number_of_favoured_aspect2_on[[name + ' mg perspective' for name in case_list_aspect2_on]],
                     name_list[2], 'mg perspective')
        add_sum_save(folder, prefix, number_of_favoured_aspect3_on_off[[name + ' mg perspective' for name in case_list_aspect3_on_off]],
                     name_list[3], 'mg perspective')
        add_sum_save(folder, prefix, number_of_favoured_aspect3_on[[name + ' mg perspective' for name in case_list_aspect3_on]],
                     name_list[4], 'mg perspective')
        add_sum_save(folder, prefix, number_of_favoured_aspect3_npv[[name + ' mg perspective' for name in case_list_aspect3_all]],
                     name_list[5], 'mg perspective')
        add_sum_save(folder, prefix, number_of_favoured_aspect3_lcoe[[name + ' mg perspective' for name in case_list_aspect3_all]],
        ...
        number = plot_x_y_stages(folder, prefix, comparison_list_x, comparison_list_y, costs_stage1, costs_stage2, costs_stage3,
                                 costs_no_mgs, number)
        number = plot_comparison_intercon(folder, prefix, comparison_list_x, comparison_list_y, costs, number, 'mg perspective')

        comparison_list_y = [title + ' global perspective' for title in
                             ['LCOE [USD/kWh] ' + item for item in list_cases]]

        number = plot_x_y_stages(folder, prefix, comparison_list_x, comparison_list_y, costs_stage1, costs_stage2, costs_stage3,
                                 costs_no_mgs, number)
        number = plot_comparison_intercon(folder, prefix, comparison_list_x, comparison_list_y, costs, number, 'global perspective')

        for y in comparison_list_y:
            number = plot_matrix(folder, prefix, costs, number, y)

save_subfolder = 'evaluation'

evaluate_nesp(save_subfolder + '/')
